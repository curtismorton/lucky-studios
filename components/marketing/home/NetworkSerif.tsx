...
type NetworkSerifProps = {
  shows: Show[];
  content: HomeContent["network"];
};

/** Network grid — film-poster show cards with serif titles, plus the open slot. */
export default function NetworkSerif({ shows, content }: NetworkSerifProps) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-24 md:px-10 md:py-32 lg:px-16">
      ...
      <div className="flex flex-wrap items-end justify-between gap-6">
        <Reveal>
          <h2 className="type-serif max-w-3xl text-[clamp(2.25rem,5.5vw,4.5rem)]">
            {content.headline}
          </h2>
        </Reveal>
        <Reveal delay={0.1}>
      ...
      <Reveal delay={0.1}>
        <p className="mt-5 max-w-2xl text-lg text-bone/65">{content.sub}</p>
      </Reveal>
      ...
      <div className="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {lineup.map((show, index) => (
          <Reveal key={show.slug} delay={index * 0.08} className="h-full">
            ...
            >
              ...
              <span className="liquid-glass tc-label absolute left-4 top-4 rounded-full px-2.5 py-1.5 text-bone/80">
                {show.genre}
              </span>
              <span
              ...
              <div className="absolute inset-x-0 bottom-0 p-5">
                <h3 className="type-serif text-3xl leading-none">{show.title}</h3>
        ...
        <Reveal delay={lineup.length * 0.08} className="h-full">
          ...
          >
            ...
            <span>
              <span className="type-serif block text-3xl italic leading-none text-bone/80 transition-colors duration-300 group-hover:text-bone">
                Your show here.
              </span>
